...
class ProvenanceGraph:
    ...
    def get_subgraph_for_order(self, order_id: str) -> nx.DiGraph:
        ...
        full_nodes = target_nodes.copy()

        # Settlement Context (other items included in the same settlements and bank txs)
        context_nodes = set()

        for s in settlements:
            # Sibling payments/refunds in the same settlement
            for pred in self.g.predecessors(s):
                if pred not in target_nodes:
                    context_nodes.add(pred)

        for btx in [n for n in target_nodes if n.startswith("bank_tx_")]:
            # Other settlements linked to the same bank tx
            for pred in self.g.predecessors(btx):
                if pred not in target_nodes:
                    context_nodes.add(pred)

        full_nodes.update(context_nodes)

        subgraph = self.g.subgraph(full_nodes).copy()
        for n in subgraph.nodes():
            subgraph.nodes[n]['is_target_evidence'] = (n in target_nodes)
