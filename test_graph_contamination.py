...
def test_subgraph_extraction():
    ...
    node_keys = list(subgraph.nodes())
    assert "order_1" in node_keys
    assert "payment_P1" in node_keys
    assert "settlement_S1" in node_keys
    assert "customer_C1" in node_keys
    assert "order_2" not in node_keys
    assert "payment_P2" not in node_keys
