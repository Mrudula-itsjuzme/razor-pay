...
def generate_case(case_id: str, case_index: int, scenario: str, base_time: datetime) -> Tuple[List, Dict]:
    records = []
    order_id = case_id
    ...
    if scenario == "CLEAN":
        ...
        order2 = Order(order_id=order_id+"_2", customer_id=f"cust_{case_index%500}", amount=amount2, created_at=base_time, status="COMPLETED")
        payment2 = Payment(payment_id=f"pay_{case_id}_2", order_id=order_id+"_2", amount=amount2, captured_at=base_time, status="CAPTURED", method="UPI")
        fee2 = Fee(fee_id=f"fee_{case_id}_2", payment_id=payment2.payment_id, type="GATEWAY", amount=fee_amt2, created_at=base_time)
        tax2 = Tax(tax_id=f"tax_{case_id}_2", payment_id=payment2.payment_id, type="GST", amount=tax_amt2, created_at=base_time)
        
        expected2 = amount2 - fee_amt2 - tax_amt2
        total_set = expected_settlement + expected2
        
        settlement = Settlement(settlement_id=f"set_{case_id}", amount=total_set, status="COMPLETED", initiated_at=base_time, reference=f"UTR{case_id}")
        items.extend([
        ...
        bank_tx = BankTransaction(bank_transaction_id=f"btx_{case_id}", amount=total_set, timestamp=base_time, reference=settlement.reference, direction="CREDIT")
        ...
        fee2 = Fee(fee_id=f"fee_dup_{case_id}", payment_id=payment.payment_id, type="GATEWAY", amount=Decimal('99.99'), created_at=base_time)
        records.append(fee2)
        settlement = Settlement(settlement_id=f"set_{case_id}", amount=expected_settlement, status="COMPLETED", initiated_at=base_time, reference=f"UTR{case_id}")
        items.append(SettlementItem(item_id=f"si_{case_id}_1", settlement_id=settlement.settlement_id, payment_id=payment.payment_id, amount=expected_settlement))
        bank_tx = BankTransaction(bank_transaction_id=f"btx_{case_id}", amount=expected_settlement, timestamp=base_time, reference=settlement.reference, direction="CREDIT")
...
def generate_dataset(num_orders: int = 2500, seed: int = 42) -> Tuple[List, List]:
    ...
    for i in range(1, num_orders + 1):
        ...
        r, c = generate_case(str(i), i, s, base_time)
        records.extend(r)
...
def generate_demo_dataset() -> Tuple[List, List]:
    ...
    for i, s in enumerate(scenarios, 1):
        r, c = generate_case(str(i * 1000), i, s, base_time)
        records.extend(r)
        cases.append((c["order_id"], c["ground_truth"]))
    return records, cases
...
def generate_adversarial_case(case_id: str, case_index: int, scenario: str, base_time: datetime) -> Tuple[List, Dict]:
    records = []
    order_id = case_id
    ...
    if scenario == "ADV_SAME_AMOUNT_WRONG_TX":
        ...
        settlement = Settlement(settlement_id=f"set_adv_{case_id}", amount=expected_settlement, status="COMPLETED", initiated_at=initiated_at, reference=f"UTR_adv_{case_id}")
        items.append(SettlementItem(item_id=f"si_adv_{case_id}_1", settlement_id=settlement.settlement_id, payment_id=payment.payment_id, amount=expected_settlement))
        bank_tx = BankTransaction(bank_transaction_id=f"btx_adv_{case_id}", amount=expected_settlement, timestamp=initiated_at + timedelta(days=2), reference=settlement.reference, direction="CREDIT")
        unrelated_btx = BankTransaction(bank_transaction_id=f"btx_adv_lure_{case_id}", amount=expected_settlement, timestamp=initiated_at + timedelta(minutes=5), reference="UNRELATED_UTR", direction="CREDIT")
        ...
        settlement = Settlement(settlement_id=f"set_adv_{case_id}", amount=short_amount, status="COMPLETED", initiated_at=initiated_at, reference=f"UTR_adv_{case_id}")
        items.append(SettlementItem(item_id=f"si_adv_{case_id}_1", settlement_id=settlement.settlement_id, payment_id=payment.payment_id, amount=short_amount))
        
        wrong_refund = Refund(refund_id=f"ref_adv_{case_id}", payment_id="pay_adv_OTHER", amount=Decimal('50.00'), status="PROCESSED", created_at=initiated_at)
        bank_tx = BankTransaction(bank_transaction_id=f"btx_adv_{case_id}", amount=short_amount, timestamp=initiated_at, reference=settlement.reference, direction="CREDIT")
        ...
        settlement = Settlement(settlement_id=f"set_adv_{case_id}", amount=expected_settlement, status="COMPLETED", initiated_at=initiated_at, reference=f"UTR_adv_{case_id}")
        items.extend([
            SettlementItem(item_id=f"si_adv_{case_id}_a", settlement_id=settlement.settlement_id, payment_id=payment.payment_id, amount=part1),
            SettlementItem(item_id=f"si_adv_{case_id}_b", settlement_id=settlement.settlement_id, payment_id="pay_adv_OTHER", amount=part2)
        ])
        ...
        settlement = Settlement(settlement_id=f"set_adv_{case_id}", amount=expected_settlement, status="COMPLETED", initiated_at=initiated_at, reference=f"UTR_adv_{case_id}")
        items.append(SettlementItem(item_id=f"si_adv_{case_id}_1", settlement_id=settlement.settlement_id, payment_id=payment.payment_id, amount=expected_settlement))
        bank_tx1 = BankTransaction(bank_transaction_id=f"btx_adv_{case_id}", amount=expected_settlement, timestamp=initiated_at, reference=settlement.reference, direction="CREDIT")
        bank_tx2 = BankTransaction(bank_transaction_id=f"btx_adv_dup_{case_id}", amount=expected_settlement, timestamp=initiated_at, reference=settlement.reference, direction="CREDIT")
        records.extend([settlement, bank_tx1, bank_tx2])

    elif scenario == "ADV_WRONG_TAX_PERFECT_SIGNATURE":
        records.remove(tax)
        wrong_tax = Tax(tax_id=f"tax_adv_wrong_{case_id}", payment_id="pay_adv_OTHER", type="GST", amount=tax_amt, created_at=created_at)
...
def generate_adversarial_dataset() -> Tuple[List, List]:
    ...
    for i, s in enumerate(scenarios, 1):
        r, c = generate_adversarial_case(f"adv_{i}", i, s, base_time)
        records.extend(r)
        cases.append((c["order_id"], c["ground_truth"]))
...
def generate_complex_dataset() -> Tuple[List, List[Tuple[str, str]]]:
    ...
    for scenario in scenarios_normal:
        for _ in range(5):
            records, case_meta = generate_case(str(i), i - 30000, scenario, base_time)
    ...
    for scenario in scenarios_adv:
        for _ in range(5):
            records, case_meta = generate_adversarial_case(f"adv_{i}", i - 30000, scenario, base_time)
            all_records.extend(records)
            all_cases.append((case_meta["order_id"], case_meta["ground_truth"]))
            i += 1
